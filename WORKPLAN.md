...
Question-level feedback is now live in quiz mode and game mode, posting to a configured Google Apps Script / Google Sheet endpoint with local-device queue fallback if the endpoint is unavailable. Feedback containing `*JGM*` is tagged for auto-apply. The script template now includes a token-protected `doGet` readback path, but the deployed Apps Script must still be updated and redeployed before future sessions can pull those items back in automatically. Game mode now opens with a filter screen, and quiz mode now runs in 20-question batches with a redo-missed option between batches.
