...
Question-level feedback is now live in quiz mode and game mode, posting to a configured Google Apps Script / Google Sheet endpoint with local-device queue fallback if the endpoint is unavailable. Feedback containing `*JGM*` is tagged for auto-apply, and the deployed Apps Script now supports token-protected `doGet` readback for future feedback-ingestion sessions (the token itself remains private and is not stored in the repo). Game mode now opens with a filter screen, and quiz mode now runs in 20-question batches with a redo-missed option between batches.
